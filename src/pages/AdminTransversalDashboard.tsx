...
const AdminTransversalDashboard: React.FC = () => {
  ...
  const handleCreateLine = async (lineData: Omit<TransportLine, 'id' | 'created_at'>) => {
    if (!db) return;
  ...
  const handleToggleLineStatus = async (lineId: string, newStatus: boolean) => {
    if (!db) return;

    const lineRef = ref(db, `transport_lines/${lineId}`);
    await update(lineRef, { is_active: newStatus });
  };
              ...
              {voyageSubTab === 'services' && (
                <div>
                    ...
                    <div className="grid md:grid-cols-3 gap-4">
                      <div className="bg-white/5 rounded-xl p-5 border border-white/10">
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-sm font-bold text-white/70">Abonnement Trimestriel</span>
                          <span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded text-xs font-bold">90 jours</span>
                        </div>
                        <input
                          type="number"
                          placeholder="100000"
                  ...
                  <div className="mt-6 p-6 rounded-xl bg-white/5 border border-white/10">
                    <div className="flex items-center justify-between mb-4">
                      <h4 className="text-lg font-bold text-white">Gestion des Lignes d'Abonnement</h4>
                      <button
                        onClick={() => setShowCreateLineModal(true)}
                        className="px-4 py-2 bg-[#10B981] hover:bg-[#059669] text-white rounded-xl font-bold transition-all flex items-center gap-2"
                      >
                    ...
                    <div className="space-y-3">
                      {transportLines.length === 0 ? (
                        <div className="text-center py-8 text-gray-400">
                          ...
                          <p>Aucune ligne créée</p>
                          <p className="text-sm mt-1">Créez votre première ligne pour DEM-DEM Express</p>
                        </div>
                      ) : (
                          ...
                          <div key={line.id} className="bg-white/5 rounded-xl p-4 border border-white/10">
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                <h5 className="text-white font-bold text-lg mb-1">{line.name}</h5>
                                <p className="text-gray-400 text-sm mb-3">{line.route}</p>
                                <div className="grid grid-cols-3 gap-3">
                                  <div className="bg-blue-500/10 rounded-lg p-2 border border-blue-500/20">
                                    <div className="text-xs text-blue-400 mb-1">Hebdomadaire</div>
                                    <div className="text-white font-bold">{line.price_weekly.toLocaleString()} F</div>
                                  </div>
                                  <div className="bg-green-500/10 rounded-lg p-2 border border-green-500/20">
                                    <div className="text-xs text-green-400 mb-1">Mensuel</div>
                                    <div className="text-white font-bold">{line.price_monthly.toLocaleString()} F</div>
                                  </div>
                                  <div className="bg-purple-500/10 rounded-lg p-2 border border-purple-500/20">
                                    <div className="text-xs text-purple-400 mb-1">Trimestriel</div>
                                    <div className="text-white font-bold">{line.price_quarterly.toLocaleString()} F</div>
                                  </div>
                                </div>
                              </div>
                              <div className="flex items-center gap-2 ml-4">
                                <button
                                  onClick={() => handleToggleLineStatus(line.id, !line.is_active)}
                                ...
                                <button
                                  ...
                                  className="px-3 py-1 bg-red-500/20 text-red-400 rounded-lg text-xs font-bold hover:bg-red-500/30 transition-all border border-red-500/30"
                                >
                                  Supprimer
                                </button>
                              </div>
...
interface CreateLineModalProps {
  onClose: () => void;
  onCreate: (lineData: Omit<TransportLine, 'id' | 'created_at'>) => Promise<void>;
}

const CreateLineModal: React.FC<CreateLineModalProps> = ({ onClose, onCreate }) => {
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-[#1E1E1E] rounded-2xl border border-gray-800 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-800">
          <h2 className="text-2xl font-black text-white uppercase">Créer une Ligne d'Abonnement</h2>
          <p className="text-gray-400 text-sm mt-1">Nouvelle ligne pour DEM-DEM Express</p>
        </div>

        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-gray-400 text-sm font-medium mb-2">Nom de la Ligne</label>
            <input
              type="text"
              value={formData.name}
          ...
          <div>
            <label className="block text-gray-400 text-sm font-medium mb-2">Trajet</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="block text-gray-400 text-sm font-medium mb-2">Prix Hebdomadaire</label>
              <input
                type="number"
                value={formData.price_weekly}
                onChange={(e) => setFormData({ ...formData, price_weekly: parseInt(e.target.value) })}
                className="w-full px-4 py-3 bg-[#2A2A2A] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#10B981]"
                required
              />
            ...
            <div>
              <label className="block text-gray-400 text-sm font-medium mb-2">Prix Mensuel</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-gray-400 text-sm font-medium mb-2">Prix Trimestriel</label>
              <input
                type="number"
                value={formData.price_quarterly}
                onChange={(e) => setFormData({ ...formData, price_quarterly: parseInt(e.target.value) })}
                className="w-full px-4 py-3 bg-[#2A2A2A] border border-gray-700 rounded-lg text-white focus:outline-none focus:border-[#10B981]"
                required
              />
              <p className="text-xs text-gray-500 mt-1">FCFA</p>
          ...
          <div className="flex items-center space-x-3">
            <input
              ...
              className="w-4 h-4 rounded border-gray-700 bg-[#2A2A2A] text-[#10B981] focus:ring-[#10B981]"
            />
