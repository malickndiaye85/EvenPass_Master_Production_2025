...
export async function getActiveTransportLines(): Promise<BusRouteDisplay[]> {
  console.log('[DEBUG-ROUTES] Starting getActiveTransportLines...');
  console.log('[DEBUG-ROUTES] Database instance:', db ? 'INITIALIZED' : 'NULL');
  ...
  try {
    ...
    console.log('[DEBUG-ROUTES] Created reference to transport_lines');

    const snapshot = await get(linesRef);
    console.log('[DEBUG-ROUTES] Snapshot exists:', snapshot.exists());

    if (!snapshot.exists()) {
      console.warn('[DEBUG-ROUTES] ⚠️ No data found in transport_lines path');
      return [];
    }

    const data = snapshot.val();
    console.log('[DEBUG-ROUTES] Raw data from Firebase:', JSON.stringify(data, null, 2));
